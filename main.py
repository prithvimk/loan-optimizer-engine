...
def load_config(filepath: str):
    ...
    for inflow in cashflow_data.get('irregular_inflows', []):
        ...
        months_diff = (inflow_date.year - sim_start_date.year) * 12 + (inflow_date.month - sim_start_date.month)
        month_index = months_diff + 1
        if month_index > 0:
            irregular_inflows_dict[month_index] = irregular_inflows_dict.get(month_index, Decimal('0')) + Decimal(str(inflow['amount']))

    cashflow = CashFlowProfile(
        monthly_income=Decimal(str(cashflow_data.get('monthly_income', '0'))),
        fixed_living_expenses=Decimal(str(cashflow_data.get('fixed_living_expenses', '0'))),
        emergency_buffer=Decimal(str(cashflow_data.get('emergency_buffer', '0'))),
        income_growth_rate_annual=Decimal(str(cashflow_data.get('income_growth_rate_annual', '0'))),
        expense_inflation_rate_annual=Decimal(str(cashflow_data.get('expense_inflation_rate_annual', '0'))),
        irregular_outflows={k: Decimal(str(v)) for k, v in cashflow_data.get('irregular_outflows', {}).items()},
    ...
    for loan_data in data.get('loans', []):
        ...
        loan = Loan(
            ...
            interest_method=InterestMethod[loan_data.get('interest_method', 'MONTHLY').upper()],
            repayment_type=RepaymentType[loan_data.get('repayment_type', 'EMI').upper()],
