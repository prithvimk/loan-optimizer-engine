...
class Reporter:
    ...
    """
    def __init__(self, optimized_history: List[Dict[str, Any]], baseline_history: List[Dict[str, Any]], cashflow, loans):
    ...
    def generate_html_report(self, output_path: str):
        ...
        summary_stats = {
            ...
            'debt_free_date': debt_free_date.strftime('%Y-%m-%d') if not pd.isna(debt_free_date) else 'N/A'
        }
        ...
        allocations.append({
            ...
            'percentage': f"{(base_expenses / base_income * 100):.1f}%" if base_income else "0%"
        })
        ...
        for _, row in m1_df.iterrows():
            ...
            allocations.append({
                ...
                'percentage': f"{(total_pay / base_income * 100):.1f}%" if base_income else "0%"
            })
        ...
        allocations.append({
            ...
            'percentage': f"{(remaining / base_income * 100):.1f}%" if base_income else "0%"
        })

        # Loan level summary
        opt_loan_grp = opt_df.groupby('loan_id').agg({'interest': 'sum', 'mandatory_payment': 'sum', 'extra_payment': 'sum', 'month_index': 'max'})
        base_loan_grp = base_df.groupby('loan_id').agg({'interest': 'sum', 'month_index': 'max'})
        ...
        for loan_id, opt_row in opt_loan_grp.iterrows():
            ...
            rollovers = opt_df[(opt_df['loan_id'] == loan_id) & (opt_df['rolled_over'] == True)].shape[0]
            ...
            loans_data.append({
                ...
                'total_payments': format_inr(opt_row['mandatory_payment'] + opt_row['extra_payment']),
